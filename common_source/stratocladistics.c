...
/*					09/14 - MCMC stratolikelihood added
/**********************************************************************/
...
for (d=(notu-1); d>=0; --d)	{
	...
	if (ancestral[d]>0 && d<notu)	foote=d;					/* do something with this!	*/
...
for (c=(notu+nodes)-1; c>=notu; --c)	{
	...
	for (d=0; d<(notu+nodes); ++d)	{
		if (ape[d]==c)	{
			if (ancestral[d]>0 && d<notu)	foote=d;	/* do something with this: we probably need two passes	*/
			fa_ma[f]=divergences[desc[f]=d][1];
			adds[f]=exponentialrandom(lambda);
			++f;
			}
		}
	dshellsort_inc_command(adds,f);	/* sort smallest increases to largest	*/
	dshellsort_inc_command(fa_ma,f);	/* sort largest increases to smallest	*/
	for (d=0; d<f; ++d)	fa_ma[d]-=adds[d];	/* move fa up a bit	*/
	divergences[c][0]=divergences[c][1]=mindarray(fa_ma,f);
...
/*
assess P[n finds in N tries | theta[i] for all i rates]
	for OTUs this is empirical (but use first stage only!): but modify N to reflect divergence!!!!
	for HTUs this is n=0 over N from divergence time
	L[freqrat] = ∏ P[ni | Ni, freqrat]
	for node, put conditional likelihoods on each rate
		turn those into posterior probabilities!
************************************************************************************/
...
{
int	d, f, sp, st, ge, ga, gd;
double	h, x, y;
double time, sts, chn;
double marg[100], geogprobs[10][100];
//double fisher=0.0f;
/* establish how many missing horizons there are based on:
		divergence time
		time scale
		sampling scale	*/

if (foote<0)	for (ge=0; ge<areas; ++ge)	geoglikes[HTU][ge]=1.0f;
else			for (ge=0; ge<areas; ++ge)	geoglikes[HTU][ge]=geoglikes[foote][ge];

for (d=0; d<f1; ++d)	{
	sp=desc[d];
	time=fabs(divergences[sp][1]-divergences[sp][0]);
	sts=(1/((double) areas)) + ((((double) (areas-1))/((double) areas)) * exp(-1*areas*iota));
	chn=(1-sts)/((double) (areas-1));
	y=0.0f;
	for (ga=0; ga<areas; ++ga)	{
		marg[ga]=0.0f;
		if (d==0)	geoglikes[HTU][ga]=1.0f;				/* clear node	*/
		geogprobs[d][ga]=0.0f;							/* P [lineage in area ga	*/
		for (gd=0; gd<areas; ++gd)	{
			if (gd==ga)	{
				marg[ga]+=geoglikes[HTU][gd]*sts;			/* prob. of going ga->ga			*/
				geogprobs[d][ga]+=geoglikes[sp][gd]*sts;	/* prob. of stasis if this present in gd	*/
				}
			else	{
				marg[ga]+=geoglikes[HTU][gd]*chn;			/* prob. of going ga->gd	*/
				geogprobs[d][ga]+=geoglikes[sp][gd]*chn;	/* prob. of shift if present in gd	*/
				}
			}	/* end search through possible descendant areas gd	*/
		x=log(marg[ga])+log(geoglikes[HTU][ga]);
		geoglikes[HTU][ga]=exp(x);
		y+=geoglikes[HTU][ga];		/* sum likelihoods to estimate posterior probability	*/
		}	/* end search through possible ancestral areas ga	*/
	for (ge=0; ge<areas; ++ge)	geogprobs[d][ge]/=y;
	}	/* end trek through descendants of node	*/

/* estimate missing horizons along the way	*/

for (d=0; d<f1; ++d)	{
	sp=desc[d];
	stratlikes[sp]=0.0f;
	for (ge=0; ge<areas; ++ge)	{
		for (st=((int) divergences[sp][(2*ge)+2]); st<=((int) divergences[sp][(2*ge)+3]); ++st)	{
			h=0.0f;
			if (st!=((int) divergences[sp][(2*ge)+2] && st!=((int) divergences[sp][(2*ge)+3])))	h+=initsampling[st][ge];	/* fix this to allow for other sampling realms later	*/
			/* proportion of stage occuppied X localities in stage	*/
			else if (divergences[sp][(2*ge)+2]==divergences[sp][(2*ge)+3])
				h=((double) stagesampling[st][ge]) * fabs(divergences[sp][(2*ge)+3]-divergences[sp][(2*ge)+2])/fabs(timescale[st+1][ge]-timescale[st][ge]);
			else if (st==((int) divergences[sp][(2*ge)+2]))
				h=((double) stagesampling[st][ge]) * fabs(timescale[st+1][ge]-divergences[sp][(2*ge)+2])/fabs(timescale[st+1][ge]-timescale[st][ge]);
			else if (st==((int) divergences[sp][(2*ge)+3]))
				h=((double) stagesampling[st][ge]) * fabs(divergences[sp][(2*ge)+2]-timescale[st][ge])/fabs(timescale[st+1][ge]-timescale[st][ge]);
			for (f=1; f<=freqs; ++f)
				/* P[theta] x P[0 finds | H possible, theta] */
				stratlikes[sp]+=exp(log(theta_mods[sp][f])+(h*log(1-thetas[st][ge][f])));
			}	/* go through discrete stages within extension	*/
		}	/* note: stages have to be within geographic units in case regional stages are misaligned temporally	*/
	}	/* end examination of species in node	*/

//return fisher;
}

/*
void dateclade_stratlikelihood_ape_addexp(long *ape, double **ranges, double **divergences, int nodes, int notu, double lambda, unsigned long *ancestral)	*/
...
{
int	b, c, d, f, g, i;
int f1, sp, ge, ga, gd, st;
int desc[50];
int foote, HTU;
double adds[50], fa_ma[50], marg[100], geogprobs[10][100];
double	h, x, y;
double	time, sts, chn;
...
for (c=(notu+nodes)-1; c>=notu; --c)	{
	...
	for (d=0; d<(notu+nodes); ++d)	{
		if (ape[d]==c)	{
			desc[f1]=d;;
			fa_ma[f1]=divergences[desc[f1]=d][1];
			if (d<notu && ancestral[d]>0)	{
				foote=d;
				if (d>0)	for (b=f1; b>0; --b)	desc[b]=desc[b-1];
				desc[0]=foote;
				}
			adds[f1]=exponentialrandom(2*lambda);
			++f1;
			}
		}	/* assign membership & ID ancestor	*/

	dshellsort_inc_command(adds,f1);		/* sort smallest increases to largest	*/
	dshellsort_inc_command(fa_ma,f1);	/* sort earliest to latest FAs	*/
	for (d=0; d<f1; ++d)	fa_ma[d]-adds[d];	/* move fa up a bit	*/
	dshellsort_inc_command(adds,f1);	/* sort smallest increases to largest	*/
	dshellsort_inc_command(fa_ma,f1);	/* sort largest increases to smallest	*/
	for (d=0; d<f1; ++d)	fa_ma[d]-=adds[d];	/* move fa up a bit	*/
	divergences[c][0]=divergences[c][1]=mindarray(fa_ma,f1);
	/* date descendants: that is the earliest time if there is no sampled ancestor	*/
	if (foote==-1)	for (d=0; d<f1; ++d)	divergences[desc[d]][0]=divergences[c][1];
	else	{
		for (d=1; d<f1; ++d)	{
			sp=desc[d];
			divergences[sp][0]-=adds[d];		/* new divergence time	*/
			if (divergences[sp][0]>ranges[foote][1])	minbr[sp]=0;	/* possible anagenesis case	*/
			}	/* go through descendants: they can diverge whenever	*/
		}

	/* redo regional stages	*/
	for (d=0; d<f1; ++d)	{
		sp=desc[d];
		for (g=0; g<areas; ++g)	{
			i=((int) divergences[sp][(2*g)+3]);
			while (timescale[g][i]>divergences[sp][0])	--i;
			divergences[sp][(2*g)+2]=i;
			}
		}		/* get stages down	*/
	...
	for (d=0; d<f1; ++d)	{
		...
		for (ga=0; ga<areas; ++ga)	{
			...
			geoglikes[HTU][ga]=exp(x);
/*			y+=geoglikes[HTU][ga];		/* sum likelihoods to estimate posterior probability	*/
			y+=geogprobs[d][ga];		/* sum likelihoods to estimate posterior probability	*/
			}	/* end search through possible ancestral areas ga	*/
		for (ge=0; ge<areas; ++ge)	geogprobs[d][ge]/=y;
		}	/* end trek through descendants of node	*/
	...
	for (d=0; d<f1; ++d)	{
		...
		stratlikes[sp]=0.0f;
		for (ge=0; ge<areas; ++ge)	{
			for (st=((int) divergences[sp][(2*g)+2]); st<=((int) divergences[sp][(2*g)+3]); ++st)	{
				h=0.0f;
				if (st!=((int) divergences[sp][(2*g)+2] && st!=((int) divergences[sp][(2*g)+3])))	h+=initsampling[st][ge];	/* fix this to allow for other sampling realms later	*/
				/* proportion of stage occuppied X localities in stage	*/
				else if (divergences[sp][(2*g)+2]==divergences[sp][(2*g)+3])
					h=((double) stagesampling[st][ge]) * fabs(divergences[sp][(2*g)+3]-divergences[sp][(2*g)+2])/fabs(timescale[st+1][ge]-timescale[st][ge]);
				else if (st==((int) divergences[sp][(2*g)+2]))
					h=((double) stagesampling[st][ge]) * fabs(timescale[st+1][ge]-divergences[sp][(2*g)+2])/fabs(timescale[st+1][ge]-timescale[st][ge]);
				else if (st==((int) divergences[sp][(2*g)+3]))
					h=((double) stagesampling[st][ge]) * fabs(divergences[sp][(2*g)+2]-timescale[st][ge])/fabs(timescale[st+1][ge]-timescale[st][ge]);
				for (f=1; f<=freqs; ++f)
					/* P[theta] x P[0 finds | H possible, theta] */
					stratlikes[sp]+=exp(log(theta_mods[sp][f])+(h*log(1-thetas[st][ge][f])));
				}	/* go through discrete stages within extension	*/
...
/** datecladerealape - Finds age of clades based on the oldest member in the clade
 ...
 1: bifucating: anagenetic loss of ancestor
 ****************************************************************************/
//void dateclade_stratlikelihood_ape_pulley(int HTU, long *ape, double **divergences, int nodes, int notu, unsigned long *ancestral)
...
{
int	a, d, f, t;
int	f1=0, ga, ge, gd, sp, st, foote=-1, test=0;
int	desc[100], fa_loc[100];
double x, y, h;
double	ble=RAND_MAX, blu=-1*RAND_MAX;
double	marg[100], div[100], geogprobs[10][100];
double	sts, chn, time;

for (ge=0; ge<areas; ++ge)	fa_loc[ge]=RAND_MAX;
for (d=0; d<nodes+notu; ++d)	{
	if (ape[d]==HTU)	{
		desc[f1]=d;	/* make list of descendants	*/
		if (d<notu && ancestral[d]>0)	{
			foote=d;	/* ancestor present: only need to redate that species	*/
			if (f1>0)	for (f=f1; f>0; --f)	desc[f]=desc[f-1];
			desc[0]=foote;
			++test;
			if (test>1)	printf("ERROR Line 5756: 2 ancestors in node %d (sp. %d & %d)",HTU-notu,desc[0],desc[1]);
			}
		div[f1]=divergences[d][0];
		if (divergences[d][1]<ble)	ble=divergences[d][1];	/* oldest divergence time among descendants		*/
		if (divergences[d][0]>blu)	blu=divergences[d][0];	/* youngest divergence time among descendants	*/
		for (ge=0; ge<areas; ++ge)	if (divergences[d][(2*ge)+2]<fa_loc[ge])	fa_loc[ge]=((int) divergences[d][(2*ge)+2]);
		++f1;
		}
	}
...
if (div[a]!=divergences[HTU][1])	{
	x=fabs(divergences[HTU][0]-divergences[HTU][1]);
	divergences[HTU][1]=div[a];				/* top of node is first divergence of taxon	*/
	/* this routine should be moot now	*/
	if (divergences[HTU][0]>=divergences[HTU][1])	{
		divergences[HTU][0]=divergences[HTU][1]-(x/2);
		}
	for (ge=0; ge<areas; ++ge)	{
		t=divergences[HTU][2*(ge+1)];
		/* count down, then count up	*/
		while (divergences[HTU][0]<timescale[t][ge] && t>0)				--t;
		while (divergences[HTU][0]>timescale[t+1][ge] && t<(stages-1))	++t;
		divergences[HTU][(2*ge)+2]=t;
		/* set to ancestor if present; otherwise, set to oldest observed	*/
		if (foote==-1)	divergences[HTU][(2*ge)+3]=fa_loc[ge];
		else			divergences[HTU][(2*ge)+3]=divergences[foote][(2*ge)+2];	/* I am fence sitting here!	*/
		if (divergences[HTU][(2*ge)+2]>=divergences[HTU][(2*ge)+3])	divergences[HTU][(2*ge)+2]=divergences[HTU][(2*ge)+3];
		}	/* get local divergence times	*/
	}	/* shift divergence date if necessary	*/
	/* EUREKA!!!! 2014-09-17: the budding vs. bifurcating dilemma is solved by keeping branching time at ble, I think!!!!!	*/

/* now, date descendants & set up stratigraphic likelihood & geographic likelihood	*/
for (ge=0; ge<areas; ++ge)	{
	if (foote==-1)	{
		for (ge=0; ge<areas; ++ge)	geoglikes[HTU][ge]=1.0f;
		for (d=0; d<f1; ++d)	{
			sp=desc[d];
			if (divergences[sp][0]!=divergences[HTU][1])	{
				divergences[sp][0]=divergences[HTU][1];
				for (ge=0; ge<areas; ++ge)	divergences[sp][2*(ge+1)]=divergences[HTU][2*(ge+1)];	/* do stage appearances	*/
				}
			}	/* end trek through descendants of node	*/
		}
	else	{
		for (ge=0; ge<areas; ++ge)	geoglikes[HTU][ge]=geoglikes[foote][ge];
		for (d=1; d<f1; ++d)	{
			sp=desc[d];
			a=0;
			while (divergences[sp][0]<divergences[foote][0])	{
				x=0.9*fabs(divergences[sp][1]-divergences[sp][0]);
				divergences[sp][0]=divergences[sp][1]-x;
				a=1;
				}
			if (a==1)	{
				for (ge=0; ge<areas; ++ge)	{
					t=divergences[sp][2*(ge+1)];
					/* count down, then count up	*/
					while (divergences[sp][0]<timescale[t][ge] && t>0)				--t;
					while (divergences[sp][0]>timescale[t+1][ge] && t<(stages-1))	++t;
					divergences[sp][(2*ge)+2]=t;
					}	/* do stage appearances	*/
				}
			}	/* end trek through descendants of node	*/
		}
	}
/* now estimate each branch's stratigraphic likelihood; the nodal branch will be done in the next clade	*/
for (d=0; d<f1; ++d)	{
	sp=desc[d];
	time=fabs(divergences[sp][1]-divergences[sp][0]);
	sts=(1/((double) areas)) + ((((double) (areas-1))/((double) areas)) * exp(-1*areas*iota));
...
if (ape[HTU]!=TREE_END)	dateclade_stratlikelihood_ape_pulley(ape[HTU], ape, divergences, notu, nodes, ancestral, thetas, theta_mods, timescale, initsampling, stagesampling, geoglikes, freqs, stratlikes, stages, areas, iota);	/* recursion is our friend	*/
}	/* do another version with stratigraphic likelihood!	*/
