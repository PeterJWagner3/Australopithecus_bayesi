...
{
int	a, b, c, d, f, g, p;
int	cc, sp, max, st1, st2, sc1, sc2, rc;
int cl[20], cr[20];
int	keyst1, keyst2;						/* these are the "swing" states between characters	*/
int mxcm;
...
pairs=lmatrix(p=(states[ch1]+states[ch2]),2);	/* state pairs between two characters		*/
prfl=lmatrix(p,2);								/* first/last appearance of character pairs	*/

/*	Find each state paired with multiple states: these are the crux	*/
keyst1=-1;
/* find the state for character 1 that is paired with the state with multiple states of character 2	*/
st1=0;
a=1+imax(states[ch1],states[ch2]);
mxcm=maxulmatrix(combos,2,a);	/* this is worth doing only if some states have multiple combos	*/
	...
	for (c=0; c<sc1; ++c)	{
		...
		for (sp=0; sp<notu; ++sp)	{
			if (matrix[sp][ch1]==keyst1 && (matrix[sp][ch2]!=UNKNOWN && matrix[sp][ch2]!=INAP))	{
				...
				if (b==0)	{
					pairs[p][0]=keyst1;					/* pair #p st. 1 found	*/
					pairs[p][1]=matrix[sp][ch2];		/* pair #p st. 2 found	*/
					prfl[p][0]=pairrng[keyst1*states[ch2]+matrix[sp][ch2]][0];	/* FA of pair p	*/
					prfl[p][1]=pairrng[keyst1*states[ch2]+matrix[sp][ch2]][1];	/* LA of pair p	*/
					++p;	/* increment pairs	*/
					}
		...
		for (st2=0; st2<states[ch2] && keyst2==-1; ++st2)	{
			if (combos[1][st2]>1)	{
				for (sp=0; sp<notu; ++sp)	{
					...
					if (matrix[sp][ch2]==st2 && matrix[sp][ch1]==keyst1)	keyst2=st2;
					}
				...
				if (keyst2>-1)	{
					...
					for (sp=0; sp<notu & rc<combos[1][keyst2]; ++sp)	{
						if (matrix[sp][ch2]==keyst2 && (matrix[sp][ch1]!=UNKNOWN && matrix[sp][ch1]!=INAP))	{
							...
							if (b==0)	{
								pairs[p][0]=matrix[sp][ch1];	/* pair #p st. 1 found	*/
								pairs[p][1]=keyst2;				/* pair #p st. 2 found	*/
								prfl[p][0]=pairrng[matrix[sp][ch1]*states[ch2]+keyst2][0];	/* FA of pair p	*/
								prfl[p][1]=pairrng[matrix[sp][ch1]*states[ch2]+keyst2][1];	/* LA of pair p	*/
								++p;	/* increment pairs	*/
					...
					for (d=0; d<p; ++d)	{
						...
						else if (pairs[d][0]==keyst1)	{
							cl[f]=d;											/* other pairs with key state 1	*/
							++f;
							}
						else if (pairs[d][1]==keyst2)	{
							cr[g]=d;											/* other pairs with key state 2	*/
							++g;
							}
						}
...
/*				for (cc=0; cc<p && (pairs[cc][0]!=keyst1 || pairs[cc][1]!=st2); cc=cc)	++cc;	*/
	/*			if (cc<p)	keyst2=st2;				/* WHY IS THIS HERE???	*/
				}	/* end test of whether "swing state" is key state 2 	*/
			}	/* end search for 2nd key state	*/
		}
	}			/* put in alternative here: but really, it does not mean anything	*/
