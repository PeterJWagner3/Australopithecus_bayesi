...
void HeuristicSearchClass::SetStartTreesUse(bool setting) { useStartTrees=setting; }
void HeuristicSearchClass::SetStartTrees(NODE **start_trees, int ntrees) {
	starttrees=AllocateTreeList(ntrees, *ntaxa);
...
void HeuristicSearchClass::HeuristicSearch(DATA * const data, NODE ** const OptTrees, int &nopttrees){
	...
	cout<<"\n\n***********************************************\nBeginning Heuristic Search ("<<reps<<" RandSeqAdd reps";
	...
	tree_list=AllocateTreeList(MaxTrees+1, *ntaxa);							// allocate tree list
	...
	if (starttrees && !useStartTrees) {								// if start trees are to be held in memory (there ARE start trees, and they are designated to be held);
		float debt_holder;
		best_debt=10e6;
		for (i=1;i<=nstarttrees;i++) {
			++nopttrees;
			CopyTree(starttrees[i], OptTrees[i], data->GetNTaxa());
			root=GetRoot(OptTrees[i], ntaxa);
	//		if (data->hasStratChr) parsimony.DateTree(root, data);
			debt_holder=parsimony.TotalDebt(root, data);
			if (debt_holder<best_debt) best_debt=debt_holder;
		}
		FreeTreeList(starttrees, &nstarttrees);
		nstarttrees=0;
		starttrees=0;
	}
	else best_debt=0;
	if (useStartTrees) reps=nstarttrees;
...
//		if (data->hasStratChr) parsimony.DateTree(root, data);		// nodes must have FA's for strat chrs to work
		...
		if (!best_debt) best_debt=best_debt_this_rep;			// if this is the first rep, the stepwise tree debt is the best overall debt
...
void HeuristicSearchClass::SingleHeuristicSearch(DATA * const data, int &nopttrees)
{
	...
	while (swapped_trees<trees_this_rep) {
		...
		cout << "\tSwapping Tree " << swapped_trees <<" of "<< trees_this_rep;
		if (swapped_trees>1) {
