...
for (r=rin; abs(ri)>0 && r>=ntaxa; r+=ri)	{
	...
	/* optimal richness is overshot */
		...
		if (r-ri>=ntaxa)
			...
			ri/=2;				/* if negative step carries below ntaxa, then just cut ri in half */
...
int unqfnd=0;				/* number of different counts (=nspec if all counts unique, =1 if all species have X finds )*/
int rin;					/* initial richness */
int	he = 0;					/* tells you if you have just halved the slope increment rate 		*/
int hr = 0;					/* tells you if you have just halved the richness increment rate 	*/
double ev = 0.000f;			/* LOOP SLOPE 		*/
double emin = 1.0f;			/* min slope		*/
...
for (r=rin; abs(ri)>0 && r>=ntaxa; r+=ri)	{
	...
	for (i=0; i<3; i++) es[i] = 0.0f;
	/* increment slope until that fails to improve likelihood or resolution limit reached */
	he=0;	/* this needs to be 0 at the start of each search for optimal evenness */
	for (ev = ein; (ev>=emin && ((pbes == 0.0f) || (bep[0] > (pbes + SUPINC)))); ev += ei) {	
		...
		if (es[0] >= bep[0]) {					/* IF BETTER THAN BEST FIT */
			...
			rs[0]=bep[0] = es[0];						/* STORE FIT */
			bep[1] = ev;								/* STORE SLOPE */
			/* if we have just moved ei/2 after ev+ei was worse, then we do not want to add ei/2 and go to ev+ei again */
			/* therefore, cut ei in half again so that we go to ev+(3ei/4), which should be the unique */
			if (he==1)	ei/=2;					
			}
		else if ((es[2] > es[1]) && (es[0] > es[1]) && ((ev-ei)>emin)) {/* TOO FAR: THE PEAK HAS BEEN PASSED */
			ev -= ei;									/* STEP BACK to where you were */
			/* Reverse direction UNLESS reversing would carry you below 1.0		*/
			if ((ev-ei)>1.0)	{
				ei*=-1;				/* STEP BACKWARD TO FIND PEAK */
				he=0;				/* we can go two steps in this direction potentially */
				}
			/* If it would, then proceed forward one half the distance		 */
			else	{
				ei/=2;
				he=1;				/* we can go only one step in this direction without repeating ev's */
				}
			es[1] = es[2];								/* SET PREVIOUS S TO IGNORE OVER STEP */
			}
		else {											/* NOT IMPROVING TRY SMALLER INCREMENT */
	...
	if (bep[0] >= brp[0]) {								/* IF BETTER THAN BEST FIT */
		...
		/* if we have just moved ri/2 after r+ri was worse, then we do not want to add ri/2 and go to r+ri again */
		/* therefore, cut ri in half again so that we go to r+(3ri/4), which should be the unique */
		if (hr==1)	ri/=2;
		}
	/* optimal richness is overshot */
/*	else if ((rs[2]>rs[1] && (rs[0]>rs[1])) && (abs(ri)>1))	{	*/
		...
		if (r-ri>=ntaxa)	{
			ri*=-1;				/* step backwards to peak */
			hr=0;				/* we can go two steps in this direction without repeating r */
			}
		else	{
			ri/=2;				/* if negative step carries below ntaxa, then just cut ri in half */
			hr=1;
...
int he=0, hr=0, hm=0;		/* whether we have just halved the increments for evenness, richness & mode */
...
for (mode=0; x>=mimin && (mode>=mdmn && mode<=mdmx)/* && ((pbms == 0.0f) || (bmp[0] > (pbms + SUPINC)))*/; mode+=mi)	{
	...
	for (r=rin; abs(ri)>0 && r>=ntaxa; r+=ri)	{
		...
		/* calculate the lowest magnitude of increase that this algorithm can use */
		if (mode>=0)	emin=pow(1.5,1/((2*ocs)-mode));
		else			emin=pow(1.5,1/((2*ocs)+mode));
		...
		for (ev=ein; ev<=mxev && ((pbes==0.0f) || (bep[0]>(pbes+SUPINC))); ev+=ei)	{
			...
			/* makes sure that ev does not drop too low */
			while ((ev+ei)<emin)	ei/=2;
