...
void MakeStratBL(NODE *tree, DATA *data) {
	...
	for (i=1;i<2**data->GetNTaxa();++i) {
		...
		s=tree[i].sister;
		if (i<=*data->GetNTaxa()) {
			if (!tree[i].isAnc && !s->isAnc) *tree[i].bl=*data->GetLInt(i)-*data->GetFInt(a->label)+1;
			else {
				if (*data->GetFInt(s->label)>=*data->GetLInt(i)) tree[i].bl=0;
				else *tree[i].bl=*data->GetLInt(i)-*data->GetFInt(s->label);
			}
		}
		else {
			if (!a)	tree[i].bl=0;
			else *tree[i].bl=*data->GetFInt(i)-*data->GetFInt(a->label)+1;
		}
	}
}
